	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while(!frontier.isEmpty()) {
			...
			for(int i = 0; i < childStates.size(); i++) {
				SearchNode child = new SearchNode(childStates.get(i), node);
				...
				if(!explored.contains(child) && !frontier.contains(child)) {
					//If we do a depth-first search we want to insert the nodes in the front of the queue
					//and if we do a breadth-first search we want to insert in the back of the queue
					if(insertFront) {
